/**
 * useTerminateEmployee — WU-05A
 *
 * TanStack mutation composable for POST /admin/employees/:id/terminate.
 *
 * Rules:
 ...
 * - EMPLOYEE_ALREADY_TERMINATED (409): employee is already in TERMINATED status
 */
...
import type { TerminateEmployeeDto } from '../interfaces/employee.types'
...
interface TerminateEmployeeVariables {
  id: string
...
export function useTerminateEmployee() {
  ...
  const mutation = useMutation<Employee, AxiosError, TerminateEmployeeVariables>({
    mutationFn: ({ id, dto }: TerminateEmployeeVariables) => employeesApi.terminate(id, dto),
    ...
    onSuccess: (_data, variables) => {
      ...
      toast.add({ title: 'Colaborador dado de baja exitosamente', color: 'success' })
    },
  ...
  return {
    /** Call to trigger termination. Throws on error — wrap with try/catch if needed. */
