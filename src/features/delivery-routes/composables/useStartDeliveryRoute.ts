/**
 * useStartDeliveryRoute — S5b (sdd delivery-routes, design.md §6.3, §7.2, §10.1)
 *
 * TanStack mutation composable for `POST /delivery-routes/:id/start`
 * (manager-only; transitions DRAFT → ACTIVE).
 *
 * Contract:
 *   - On success: invalidate BOTH `deliveryRouteQueryKeys.detail(tenantId, id)`
 *     AND `deliveryRouteQueryKeys.listPrefix(tenantId)`. Fires the Spanish
 *     "Ruta iniciada" toast.
 *   - On 409 `DELIVERY_ROUTE_STOP_SALE_ALREADY_ON_ACTIVE_ROUTE`: specific
 *     conflict toast + invalidate BOTH detail and listPrefix (resync the stale
 *     DRAFT status) + NO auto-retry (TanStack mutations default to `retry: 0`;
 *     the manager must resolve the conflicted sale and re-start manually).
 *   - Other errors route through `surfaceDeliveryRouteError(error, 'toast')`.
 *   - NO optimistic writes, NO setQueryData (payment-details convention).
 *   - Returns `{ mutateAsync, isPending, error }` (mirrors the other mutations).
 *
 * Pure handlers (`handleStartSuccess`, `handleStartError`) are extracted as
 * named exports so the co-located spec can drive them with mock deps — no
 * Pinia, no QueryClient, no toast runtime in the unit test (extract-before-mock).
 */

import { useMutation, useQueryClient } from '@tanstack/vue-query'
import type { AxiosError } from 'axios'
import { computed } from 'vue'
import { useAuthStore } from '@/features/auth/stores/useAuthStore'
import { deliveryRouteQueryKeys } from '@/core/shared/constants/query-keys'
import { DELIVERY_ROUTE_COPY } from '../copy'
import { deliveryRoutesApi } from '../api/delivery-routes.api'
import {
  surfaceDeliveryRouteError,
  extractDeliveryRouteErrorCode,
  type DeliveryRouteErrorSurface,
} from '../interfaces/errors'
import type { DeliveryRouteResponseDto } from '../interfaces/delivery-route.types'

// useToast is auto-imported by @nuxt/ui/vite plugin (unplugin-auto-import).
declare const useToast: () => {
  add: (options: {
    title: string
    description?: string
    color?: 'success' | 'error' | 'warning' | 'primary' | 'neutral'
  }) => void
}

/**
 * StartMutationDeps — Side-effect collaborators the mutation delegates to.
 * Pure handlers accept these so the unit test asserts against `vi.fn()` mocks
 * without any runtime.
 */
export interface StartMutationDeps extends DeliveryRouteErrorSurface {
  /** Invalidate the per-tenant detail query so the next read is fresh. */
  invalidateDetail: (args: { queryKey: readonly unknown[] }) => void
  /** Invalidate the per-tenant list prefix query. */
  invalidateList: (args: { queryKey: readonly unknown[] }) => void
}

/**
 * PURE success handler — invalidates detail + listPrefix + fires Spanish toast.
 * Exported for direct unit testing.
 */
export function handleStartSuccess(
  tenantId: string,
  id: string,
  deps: StartMutationDeps,
): void {
  deps.invalidateDetail({ queryKey: deliveryRouteQueryKeys.detail(tenantId, id) })
  deps.invalidateList({ queryKey: deliveryRouteQueryKeys.listPrefix(tenantId) })
  deps.addToast({ title: DELIVERY_ROUTE_COPY.toasts.startSuccess, color: 'success' })
}

/**
 * PURE error router — the 409 conflict is special-cased (§10.1): refetch BOTH
 * detail + list so the stale DRAFT status resyncs, then surface the conflict
 * toast. Every other domain / transport error flows through the shared
 * `surfaceDeliveryRouteError(error, 'toast')` helper (S5a REFACTOR target).
 */
export function handleStartError(
  error: unknown,
  tenantId: string,
  id: string,
  deps: StartMutationDeps,
): void {
  if (
    extractDeliveryRouteErrorCode(error) ===
    'DELIVERY_ROUTE_STOP_SALE_ALREADY_ON_ACTIVE_ROUTE'
  ) {
    deps.invalidateDetail({ queryKey: deliveryRouteQueryKeys.detail(tenantId, id) })
    deps.invalidateList({ queryKey: deliveryRouteQueryKeys.listPrefix(tenantId) })
  }
  surfaceDeliveryRouteError(error, 'toast', deps, 'No se pudo iniciar la ruta')
}

/**
 * useStartDeliveryRoute — composable wrapper.
 *
 * Returns the same shape as the other delivery-route mutations:
 *   - mutateAsync(id) — call to trigger the start
 *   - isPending       — true while the request is in-flight
 *   - error           — the last error, or null
 */
export function useStartDeliveryRoute() {
  const queryClient = useQueryClient()
  const authStore = useAuthStore()
  const tenantId = computed(() => authStore.currentTenantId)
  const toast = useToast()

  const mutation = useMutation<DeliveryRouteResponseDto, AxiosError, string>({
    mutationFn: (id) => deliveryRoutesApi.start(id),

    onSuccess: (_response, id) => {
      handleStartSuccess(tenantId.value, id, {
        invalidateDetail: ({ queryKey }) => {
          void queryClient.invalidateQueries({ queryKey })
        },
        invalidateList: ({ queryKey }) => {
          void queryClient.invalidateQueries({ queryKey })
        },
        addToast: (t) => toast.add(t),
      })
    },

    onError: (error: AxiosError, id) => {
      handleStartError(error, tenantId.value, id, {
        invalidateDetail: ({ queryKey }) => {
          void queryClient.invalidateQueries({ queryKey })
        },
        invalidateList: ({ queryKey }) => {
          void queryClient.invalidateQueries({ queryKey })
        },
        addToast: (t) => toast.add(t),
      })
    },
  })

  return {
    /** Call to trigger the start. Throws on error — wrap with try/catch if needed. */
    mutateAsync: mutation.mutateAsync,
    /** True while the request is in-flight */
    isPending: mutation.isPending,
    /** The last error, or null */
    error: mutation.error,
  }
}
