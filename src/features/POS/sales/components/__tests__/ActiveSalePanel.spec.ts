    ...
    global: {
      ...
      stubs: {
        ...
        PromocionesDisponiblesAccordion: {
          ...
          template:
            '<div data-testid="promociones-disponibles-accordion-stub" '
...
describe('ActiveSalePanel customer slot', () => {
  it('renders empty state with Asignar cliente trigger as solid button and opens slideover event', async () => {
...
describe('ActiveSalePanel C.4 — PromocionesDisponiblesAccordion wiring', () => {
  const samplePromotions: ApplicablePromotion[] = [
    ...
    it('does NOT mount the accordion when applicablePromotions is undefined or empty', () => {
      ...
      expect(wrapperEmpty.find('[data-testid="promociones-disponibles-accordion-stub"]').exists()).toBe(false)

      const wrapperUndef = mountPanel(makeDraft(), false, {})
      expect(wrapperUndef.find('[data-testid="promociones-disponibles-accordion-stub"]').exists()).toBe(false)
    })
    ...
    it('mounts the accordion ABOVE SaleTotalsFooter and passes the promotions, loading, and appliedIds props', () => {
      ...
      const accordionNode = wrapper.find('[data-testid="promociones-disponibles-accordion-stub"]').element
