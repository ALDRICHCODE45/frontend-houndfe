...
## S4b — CustomersView sales-history wiring (work unit `tdd-rebuild-s4b-view-wiring`)

Same worktree/branch; S4a base `31fc74e` (clean tree, pre-S4b production; focused baseline 11/11 green). Source reference (final-content only, NOT cherry-picked): `d745a319`. Token `sha256:ca014852fe4898df9ad88745dcc77181636213f71a6e89417d5ff845eb84cf25`. Focused command throughout: `pnpm test:unit --run src/features/POS/customers/views/__tests__/CustomersView.test.ts`. Date: 2026-09-05.

### TDD Cycle Evidence (strict TDD)

| Step | When (local) | Commit | Tree | Command | Exit | Result |
| --- | --- | --- | --- | --- | --- | --- |
| Baseline | 05:44:53 | `31fc74e` (base) | `31fc74e` | focused | 0 | 1 file / 11 tests passed; tree clean |
| RED | 05:48:07 | `fee13ac` `test(customers): define customer history view wiring` | `1a28f8c` | focused | 1 | 5 failed / 9 passed (14) against pre-S4b production, all for S4-contract reasons: `read:Sale`-only kebab → `expected [] to deeply equal ['Ver historial de ventas']` (no history item); `canReadSales` to grid → `expected undefined to be true`; slideover closed-by-default → `exists()` false (not composed); table action opens w/ identity → `expected undefined to be defined` (no item); grid forwarding → `Cannot call props on an empty VueWrapper` (slideover absent) |
| GREEN | 05:55:36 | `e3ce205` `feat(customers): expose sales history actions` | `9ad730c` | identical | 0 | 14/14 passed. Minimal production only in `CustomersView.vue`: `canReadSales` + `canShowCustomerActions` (= update ∥ delete ∥ read:Sale), independent `historyCustomer`/`isHistoryOpen` `shallowRef`s + `handleOpenHistory`, kebab `Ver historial de ventas` item, `CustomerSalesHistorySlideover` composition (`v-model:open` + `:customer`), grid `:can-read-sales` + `@view-history`, `v-if` union swap. Edit/detail/delete state and handlers untouched (create/edit slideovers, ConfirmModal, mutations verbatim) |
| TRIANGULATE | 05:56:04 | `83d57c3` `test(customers): triangulate customer history view wiring` | `3bd8c7e` | identical | 0 | 16/16 passed on first run — **no production correction driven**. Held-back: explicit no-permission kebab absence (`flat` empty), close/reopen A→B identity (`update:open` false → history closed with edit AND create slideovers still closed; reopening row B shows `cust-B` while row A selection is replaced), table/card parity + `read:Sale`-only visibility already asserted by RED tests |
| REFACTOR | 05:56:12 | (no commit) | `3bd8c7e` | identical + `npx vue-tsc --build` | 0 / 0 | 16 passed; type-check clean, no generated drift. `no refactor warranted`: state is minimal (2 `shallowRef`s + 1 handler), no duplication introduced |
| Full suite | 05:56:39 | `83d57c3` (candidate) | `3bd8c7e` | `pnpm test:unit --run` (full) | 0 | 368 files / 5873 tests passed (+5 vs S4a's 368/5868) |

### Notes

- RED-commit caveat (recorded, not hidden): commit `fee13ac` was written with incomplete test infrastructure — three mock edits silently no-oped on indentation mismatch (slideover stub, grid-mock `canReadSales`/`view-history` declarations, Nuxt-UI subpath stubs). The missing stubs were completed alongside GREEN. The RED diagnostic above is nonetheless genuine: it ran at 05:48:07 against pre-S4b production (`e3ce205` did not exist) and failed for the exact S4 contract reasons. No prior commits were rewritten; no tests were edited to pass production.
- Legacy `reka-dropdown-menu-trigger` HTML-internals assertions in the permission-gating describe were superseded in RED by the public `items`-prop contract per REQ-CSH-ENT-001/003 (kebab visibility is now the union `update ∥ delete ∥ read:Sale`; the old "hidden when lacking update AND delete" contract is obsolete under the new spec). Matches reference `d745a319` final content.
- Auto-import resolution discovered: `UDropdownMenu`/`UButton` resolve from `@nuxt/ui/runtime/components/*.vue` (not `@nuxt/ui/components/*`); both subpaths stubbed so the kebab `items` are assertable without Reka internals.
- Scope check (REFACTOR): `git diff 31fc74e --name-only` shows only `CustomersView.vue` + its co-located test; no router/sidebar/ability-registry/API-adapter files touched.
