...
## S4b — CustomersView sales-history wiring (work unit `tdd-rebuild-s4b-view-wiring`) — SUPERSEDING REDO

Same worktree/branch; pre-S4b base `31fc74e` (clean, focused baseline 11/11). Source reference (final-content only, NOT cherry-picked): `d745a319`. Focused command throughout: `pnpm test:unit --run src/features/POS/customers/views/__tests__/CustomersView.test.ts`. Date: 2026-09-05 (redo).

**Why this redo exists:** fresh `sdd-verify` proved that the required S4b test-harness mock declarations (slideover stub, grid `canReadSales`/`view-history` prop+emit declarations, correct `@nuxt/ui` + `@nuxt/ui/runtime` subpath stubs) first appear in TRIANGULATE commit `83d57c3`, not in RED `fee13ac` — so the committed GREEN checkpoint `e3ce205` was not credible as a strict-TDD process artifact. Functional evidence was never in question (11/11 requirements, 22/22 scenarios, 5873 tests, build, lint, diff-check). This redo is **additive**: no amend/reset/rebase/drop of prior commits; `fee13ac → e3ce205 → 83d57c3` remain in history as the invalidated attempt. Final source/test content of this redo is **byte-identical** to the previously recorded final blobs (`CustomersView.vue` sha256 `f4e9e514…`, `CustomersView.test.ts` sha256 `5894da69…`, verified with `cmp` against blobs preserved at attempt start), so final behavior equals the prior candidate and only the process evidence is superseded.

### Superseding TDD Cycle Evidence (strict TDD)

| Step | When (local) | Commit | Tree | Command | Exit | Result |
| --- | --- | --- | --- | --- | --- | --- |
| Preparation | 06:26 | `6e3406f` `chore(customers): reset S4b for strict TDD redo` | `beed6058` | focused | 0 | Both S4b files restored verbatim to pre-S4b base `31fc74e` (rollback-to-baseline preparation, no behavior change); baseline focused run 11/11 passed; tree clean |
| RED | 06:28 | `d66a37d` `test(customers): redefine customer history view wiring` | `d3c542d6` | focused | 1 | 5 failed / 9 passed (14) against pre-S4b production, **with the complete mock scaffolding already in the RED commit** (slideover stub, grid `canReadSales`/`view-history` declarations, all four `@nuxt/ui` + `@nuxt/ui/runtime` subpath stubs) — all five genuine S4-contract failures: read:Sale-only kebab `expected [] to deeply equal ['Ver historial de ventas']`; grid `canReadSales` `expected undefined to be true`; slideover closed-by-default `expected false to be true` (not composed); table action `expected undefined to be defined` (no item); grid forwarding `Cannot call props on an empty VueWrapper` (slideover absent) |
| GREEN | 06:29 | `6629c83` `feat(customers): restore customer history view wiring` | `88166e69` | identical | 0 | 14/14 passed. **Production-only commit** (`CustomersView.vue` +29/−7): `canReadSales` + `canShowCustomerActions` (= update ∥ delete ∥ read:Sale), independent `historyCustomer`/`isHistoryOpen` `shallowRef`s + `handleOpenHistory`, kebab `Ver historial de ventas` item, `CustomerSalesHistorySlideover` composition (`v-model:open` + `:customer`), grid `:can-read-sales` + `@view-history`. Commit contains only `CustomersView.vue` (verified via `git diff HEAD^ HEAD --name-only`) — zero test or mock edits |
| TRIANGULATE | 06:30 | `22c093c` `test(customers): retriangulate customer history view wiring` | `da56fb56` | identical | 0 | 16/16 passed on first run — **no production correction driven**. Held-back tests only (+53 lines): explicit no-permission kebab absence (`flat` empty); A→B close/reopen identity (`update:open` false closes history with edit AND create slideovers verified closed throughout — edit/detail independence — then reopening row B shows `cust-B` while row A selection is replaced). Table/card parity and `read:Sale`-only visibility were already RED-covered (`compact union` + `canReadSales` + grid forwarding) |
| REFACTOR | 06:31 | (no commit) | `da56fb56` | focused + `npx vue-tsc --build` | 0 / 0 | 16 passed; type-check clean. `no refactor warranted`: final production is byte-identical to the previously verified final blob; state is minimal (2 `shallowRef`s + 1 handler) |
| Full suite | 06:31 | `22c093c` (candidate) | `da56fb56` | `pnpm test:unit --run` (full) | 0 | 368 files / 5873 tests passed (+5 vs S4a's 368/5868) — identical totals to the prior candidate |

### Redo notes

- **Credibility guards applied:** the RED commit was assembled by a scripted transform of the final test blob that asserted (before writing) that all mock scaffolding markers and all five minimal contract tests are present, and that the held-back tests are absent. The GREEN commit was verified to touch only `CustomersView.vue`. The RED run executed before any S4b production existed in the working tree (preparation commit `6e3406f` restored base `31fc74e`).
- **History integrity:** preparation/RED/GREEN/TRIANGULATE commits are purely additive on top of `8b530c5`; the invalidated `fee13ac → e3ce205 → 83d57c3` sequence is retained unchanged. Legacy `reka-dropdown-menu-trigger` HTML-internals assertions remain superseded by the public `items`-prop contract per REQ-CSH-ENT-001/003.
- Scope check: `git diff 31fc74e 22c093c --name-only` shows only `CustomersView.vue` + its co-located test (same +220/−30 = 250 work-unit diff as before). Generated `auto-imports.d.ts`/`components.d.ts` drift after test runs was proven validation-generated and restored via `git checkout --` after every step, never committed.
- The failed verify attempt's untracked `verify-report.md` (sha256 `1b95ac854c4d0392b8968abea03f61fc18c3dfe1ecc936c95213a42e5c391f2f`) was preserved unchanged throughout; it belongs to the failed attempt and will be replaced by the verifier.
